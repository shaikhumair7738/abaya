            ...
            <div class="ibox-content" style="min-height: 365px;">
                ...
                <table class="table table-striped table-bordered" style="margin-top: 26px;">
                    ...
                    <th class="text-right">{$_L['Balance']}</th>
                        ...
                        <tr>
                            <td>{$account_name}</td>
                            <td class="text-right"><span class="amount{if $ds['balance'] < 0} text-red{/if}">{$net_worth}</span></td>
                        </tr>
...
<div class="row" id="sort_3">
  ...
  <div class="ibox-content">
                ...
                <div id="invoice_stats" style="display: none;">
								...
								<a href="{$_url}invoices/followup/" class="btn btn-primary btn-xs pull-right"><i class="fa fa-list"></i> Follow Up Invoices</a>
                <h4>Follow Up</h4>
                ...
                <table class="table table-bordered table-hover">
                    ...
                    {foreach $followup as $fp}
                        <tr>
                            <td><a href="{$_url}invoices/view/{$fp['id']}/" target="_blank">{$fp['invoicenum']}{if $fp['cn'] neq ''} {$fp['cn']} {else} {$fp['id']} {/if}</a> </td>
                            <td><a href="{$_url}contacts/view/{$fp['userid']}/" target="_blank">{$fp['account']}</a> </td>
                            <td>{$fp['phone']}</td>
                            <td class="amount">{$fp['total']}</td>
                            <td>{date( $_c['df'], strtotime($fp['date']))}</td>
                            <td>{date( $_c['df'], strtotime($fp['duedate']))}</td>
                            <td>


                                {if $fp['status'] eq 'Unpaid'}
                                    <span class="label label-danger">{ib_lan_get_line($fp['status'])}</span>
                                {elseif $fp['status'] eq 'Paid'}
                                    <span class="label label-success">{ib_lan_get_line($fp['status'])}</span>
                                {elseif $fp['status'] eq 'Partially Paid'}
                                    <span class="label label-info">{ib_lan_get_line($fp['status'])}</span>
                                {elseif $fp['status'] eq 'Cancelled'}
                                    <span class="label">{ib_lan_get_line($fp['status'])}</span>
                                {else}
                                    {ib_lan_get_line($fp['status'])}
                                {/if}
