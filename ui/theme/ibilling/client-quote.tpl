    ...
    <div class="panel-body">
          ...
          <div class="row">
              ...
              <div class="bill-to">
                ...
                <address>
                  ...
                  {/if}
                    ...
                    {$a['state']} - {$a['zip']} <br>
                    {$a['country']}<br>
										<strong>{$_L['Phone']}:</strong> {$a['phone']}<br>
										<strong>{$_L['Email']}:</strong> {$a['email']}<br>
										<strong>{$_L['gst']}:</strong> {$a['gst_no']}
                      {foreach $cf as $cfs}
												<br><strong>{$cfs['fieldname']}: </strong> {get_custom_field_value($cfs['id'],$a['id'])}
                      {/foreach}
										{$x_html}
                </address>
							</div>
            ...
            <div class="col-md-6">
                ...
                <h2> {$_L['Total']}: {$_c['currency_code']} {number_format($d['subtotal'],2,$_c['dec_point'],$_c['thousands_sep'])} </h2>
							</div>
            </div>
          </div>
        </div>
						...
						<tbody>{$i = 1}
							{foreach $items as $item}
								<tr>
									...
									<td class="text-center amount" data-a-sign="{if $d['currency_symbol'] eq ''} {$_c['currency_code']} {else} {$d['currency_symbol']}{/if} ">{$item['subtotal'] - $item['total']}</td>
									<td class="text-center amount" data-a-sign="{if $d['currency_symbol'] eq ''} {$_c['currency_code']} {else} {$d['currency_symbol']}{/if} ">{$item['subtotal']}</td>
								</tr>
							{/foreach}
						</tbody>
					</table>
							...
							<table class="table h5 text-dark">
								<tbody>
									...
									<tr>
											...
											<td class="text-left">{$_c['currency_code']} {number_format($d['subtotal']-$d['total'],2,$_c['dec_point'],$_c['thousands_sep'])}</td>
									</tr>
